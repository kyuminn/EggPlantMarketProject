...
<form:form modelAttribute="item" action="${pageContext.request.contextPath}/item/edit/${itemId}" method="post" encType="multipart/form-data">
	<h1>판매글 수정</h1>
	<hr/>
	<table class="table table-borderless">
			<tr>
				<td rowspan="4" width=300px height=300px>
					<div id='View_area' style='position:relative; width: 100%; height:100%; color: black; border: 0px solid black; dispaly: inline; '>
						<img id="preview" src="${storedThumbnailPath}" width=300px height=300px>
					</div>
				</td>
				<th>카테고리</th>
				<td>
			...
			<tr>
				<th>제품명</th>
				<td><form:input path="name" value="${item.name }"/>
					<form:errors path="name"/>
				</td>
			</tr>
			<tr>
				<th>가격</th>
				<td><form:input path="price" value="${item.price }"/>
					<form:errors path="price"/>
				</td>
			</tr>
			<tr>
				<th>대표 상품 이미지</th>
				<td><input type="file" name="uploadFile" id="uploadFile" onchange="previewImage(this,'View_area')"><br>
				<input type="button" id="remove" onclick="rmv()" value='기존 썸네일 삭제'>
				</td>
			</tr>
			<tr>
